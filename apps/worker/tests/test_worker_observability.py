...
def test_run_metrics_cover_required_operational_signals_without_content():
    state = SimpleNamespace(
        run_id=uuid4(), research_depth=ResearchDepth.STANDARD,
        snapshots=[SimpleNamespace(access_status="FETCHED", metadata={"fetch_latency_ms": 40, "cache_hit": True}, parser_name="trafilatura"), SimpleNamespace(access_status="INACCESSIBLE", metadata={"fetch_latency_ms": 80}, parser_name=None)],
...
def test_trace_outputs_allow_only_aggregate_fields():
    ...
    run_tree = RunTree()
    SafeTrace(run_tree).add_outputs({"completed_stage_count": 3, "source_content": "private"})
