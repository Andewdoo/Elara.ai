...
export function HistoryList({ savedOnly = false }: { savedOnly?: boolean }) {
  ...
  return <div className="grid gap-4">
    {!savedOnly && <Card><CardHeader><CardTitle>History filters</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <label className="flex items-center rounded-md border bg-white px-3"><Search className="h-4 w-4 text-muted-foreground"/><input className="min-w-0 flex-1 px-2 py-2 text-sm outline-none" value={query} onChange={resetPage(setQuery)} placeholder="Search runs or verdicts" /></label>
      <select className="rounded-md border bg-white px-3 py-2 text-sm" value={status} onChange={resetPage(setStatus)}><option value="all">All statuses</option>{["QUEUED", "VALIDATING", "DECOMPOSING", "RESEARCHING", "EXTRACTING", "ANALYZING_PROVENANCE", "SCORING", "SYNTHESIZING", "AUDITING", "COMPLETED", "FAILED", "CANCELLED"].map((value) => <option key={value} value={value}>{value.toLowerCase().replaceAll("_", " ")}</option>)}</select>
      <select className="rounded-md border bg-white px-3 py-2 text-sm" value={depth} onChange={resetPage(setDepth)}><option value="all">All depths</option>{["QUICK", "STANDARD", "DEEP"].map((value) => <option key={value} value={value}>{value.toLowerCase()}</option>)}</select>
      <select className="rounded-md border bg-white px-3 py-2 text-sm" value={sort} onChange={resetPage(setSort)}><option value="recent">Most recent</option><option value="oldest">Oldest first</option><option value="confidence">Confidence</option></select>
    ...
    <Card><CardHeader><CardTitle>{savedOnly ? "Saved reports" : "Verification history"}</CardTitle></CardHeader><CardContent className="grid gap-3">
      {loading || history.isLoading ? <p className="text-sm text-muted-foreground">Loading account history…</p> : !user ? <p className="text-sm text-muted-foreground">Sign in to view your report history.</p> : history.error ? <p role="alert" className="text-sm text-red-700">{history.error.message}</p> : history.data?.items.length ? history.data.items.map((item) => <article key={item.run_id} className="grid gap-3 rounded-md border p-4 md:grid-cols-[1fr_auto]">
        <div><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>{item.status}</span><span>{item.research_depth}</span>{item.saved_at && <span className="text-primary">Saved</span>}</div><Link className="mt-1 block font-semibold hover:text-primary" href={`/report/${item.run_id}`}>{item.title ?? item.submitted_text_preview ?? "Untitled verification"}</Link><p className="mt-1 text-sm text-muted-foreground">{item.verdict ?? "No verdict yet"}{item.verdict_confidence == null ? "" : ` · confidence ${item.verdict_confidence}`}</p><p className="mt-1 text-xs text-muted-foreground">Created {new Date(item.created_at).toLocaleString()}</p></div>
