...
export function SourceGraph({ graph, claims, onSourceSelect }: { graph: SourceGraphRecord; claims: ReportRecord["atomic_claims"]; onSourceSelect?: (sourceId: string) => void }) {
  ...
  if (graph.nodes.length === 0) return <p className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">No source dependency graph records were stored.</p>;
  return <div className="grid gap-3">
    ...
    <div aria-hidden="true" className="h-[430px] overflow-hidden rounded-lg border bg-white"><ReactFlow nodes={nodes} edges={edges} fitView nodesDraggable={false} onEdgeClick={(_, edge) => setSelectedEdgeId(edge.id)} onNodeClick={(_, node) => { const sourceId = graph.nodes.find((item) => item.id === node.id)?.data.sourceId; if (typeof sourceId === "string") onSourceSelect?.(sourceId); }}><MiniMap pannable zoomable/><Controls/><Background/></ReactFlow></div>
    <div className="rounded-lg border bg-white p-3 text-sm" aria-label="Accessible source graph summary"><p className="font-semibold">Graph summary</p><p className="mt-1 text-muted-foreground">{nodes.length} visible nodes and {edges.length} visible relationships.</p><ul className="mt-2 grid gap-1 text-foreground">{visibleGraphNodes.filter((node) => typeof node.data.sourceId !== "string").map((node) => <li key={node.id}>{node.type}: {node.label}</li>)}</ul><details className="mt-3 rounded-md border p-2"><summary className="cursor-pointer font-medium">Graph links ({visibleGraphNodes.filter((node) => typeof node.data.sourceId === "string").length + edges.length})</summary><ul className="mt-2 grid gap-1">{visibleGraphNodes.map((node) => { const sourceId = node.data.sourceId; return typeof sourceId === "string" ? <li key={node.id}><button type="button" className="text-left text-primary underline" onClick={() => onSourceSelect?.(sourceId)}>{node.type}: {node.label}</button></li> : null; })}{edges.map((edge) => <li key={edge.id}><button type="button" className="text-left text-primary underline" onClick={() => setSelectedEdgeId(edge.id)}>{edge.source} → {edge.target}: {String(edge.label ?? "relationship")}</button></li>)}</ul></details></div>
