...
export function SourceGraph({ graph, claims, onSourceSelect }: { graph: SourceGraphRecord; claims: ReportRecord["atomic_claims"]; onSourceSelect?: (sourceId: string) => void }) {
  ...
  return <div className="grid gap-3">
    ...
    <div className="rounded-lg border bg-white p-3 text-sm" aria-label="Accessible source graph summary"><p className="font-semibold">Graph summary</p><p className="mt-1 text-muted-foreground">{nodes.length} visible nodes and {edges.length} visible relationships.</p><ul className="mt-2 grid gap-1">{visibleGraphNodes.map((node) => { const sourceId = node.data.sourceId; return <li key={node.id}>{typeof sourceId === "string" ? <button type="button" className="text-left text-primary underline" onClick={() => onSourceSelect?.(sourceId)}>{node.type}: {node.label}</button> : `${node.type}: ${node.label}`}</li>; })}{edges.map((edge) => <li key={edge.id}><button type="button" className="text-left text-primary underline" onClick={() => setSelectedEdgeId(edge.id)}>{edge.source} → {edge.target}: {String(edge.label ?? "relationship")}</button></li>)}</ul></div>
