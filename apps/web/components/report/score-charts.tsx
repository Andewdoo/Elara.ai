...
function Chart({ title, data, dataKey, color, domain }: { title: string; data: Array<Record<string, unknown>>; dataKey: string; color: string; domain?: [number, number] }) {
  return <div className="h-72 rounded-lg border bg-white p-3"><p className="mb-2 text-sm font-semibold">{title}</p>{data.length ? <><div aria-hidden="true" className="h-[88%]"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ left: 28 }}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" domain={domain}/><YAxis type="category" dataKey="label" width={118} tick={{ fontSize: 10 }}/><Tooltip/><Bar dataKey={dataKey} fill={color} radius={[0, 4, 4, 0]}/></BarChart></ResponsiveContainer></div><AccessibleTable title={title} data={data} keys={[dataKey]}/></> : <p className="text-sm text-muted-foreground">No matching server calculation records were stored.</p>}</div>;
}

function DualChart({ title, data }: { title: string; data: Array<Record<string, unknown>> }) {
  return <div className="h-72 rounded-lg border bg-white p-3"><p className="mb-2 text-sm font-semibold">{title}</p>{data.length ? <><div aria-hidden="true" className="h-[88%]"><ResponsiveContainer width="100%" height="100%"><BarChart data={data}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="claim" tick={{ fontSize: 10 }}/><YAxis/><Tooltip/><Legend/><Bar dataKey="supporting" fill="#0f766e"/><Bar dataKey="contradicting" fill="#be123c"/></BarChart></ResponsiveContainer></div><AccessibleTable title={title} data={data} keys={["supporting", "contradicting"]}/></> : <p className="text-sm text-muted-foreground">No claim-level evidence balance records were stored.</p>}</div>;
}

function NumericalChart({ data }: { data: Array<Record<string, unknown>> }) {
  return <div className="min-h-72 rounded-lg border bg-white p-3"><p className="mb-2 text-sm font-semibold">Numerical audit</p>{data.length ? <><div aria-hidden="true" className="h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={data}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" tick={{ fontSize: 10 }}/><YAxis/><Tooltip/><Legend/><Bar dataKey="claimed" name="Claimed value" fill="#d97706"/><Bar dataKey="computed" name="Source-computed value" fill="#7c3aed"/></BarChart></ResponsiveContainer></div><AccessibleTable title="Numerical audit" data={data} keys={["claimed", "computed", "units", "denominator", "period"]}/></> : <p className="text-sm text-muted-foreground">No numerical calculation records were stored.</p>}</div>;
}

function AccessibleTable({ title, data, keys }: { title: string; data: Array<Record<string, unknown>>; keys: string[] }) {
  return <table className="sr-only"><caption>{title} data from server calculation records</caption><thead><tr><th scope="col">Item</th>{keys.map((key) => <th scope="col" key={key}>{key.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{data.map((row, index) => <tr key={`${String(row.label ?? row.claim)}-${index}`}><th scope="row">{String(row.label ?? row.claim ?? index + 1)}</th>{keys.map((key) => <td key={key}>{String(row[key] ?? "not recorded")}</td>)}</tr>)}</tbody></table>;
}
